...
testcase_timeout="${TESTCASE_TIMEOUT-0}"
case $testcase_timeout in
    (''|*[!0-9]*)
        eprintf 'Warning: ignoring non-numeric TESTCASE_TIMEOUT=%s\n' \
            "$testcase_timeout"
        testcase_timeout=0
        ;;
esac
...
testcase() {
    ...
    timed_out="false"
    set +e
    timeout_flag_file="$test_lineno.timeout"
    ...
    if "$timed_out"; then
        ...
        if "$should_succeed"; then
            eprintf '%s:%d: %s: timed out after %ss\n' \

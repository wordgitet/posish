...
static int run_arithmetic_expansion(const char *expr, char **out_value,
                                    int *status_out, struct shell_state *state) {
    char *expanded_expr;
    long value;
    char text[64];
...
static int append_parameter(const char *name, size_t nlen, struct shell_state *state,
    ...
    while (in[i] != '\0') {
        ...
        if (in[i] == '$') {
            if (in[i + 1] == '{') {
                ...
                if (j > i + 2) {
                    if (append_braced_parameter(in + i + 2, j - (i + 2), state,
                                                &buf, &len, &cap) != 0) {
                        free(buf);
                        return -1;
                    }
            ...
            if (in[i + 1] == '(') {
                if (in[i + 2] == '(') {
                    ...
                    if (run_arithmetic_expansion(expr, &value, &cmd_status,
                                                 state) != 0) {
                        free(expr);
                        free(buf);
                        return -1;
            ...
            if (isalpha((unsigned char)in[i + 1]) || in[i + 1] == '_') {
                ...
                append_parameter(in + i + 1, j - (i + 1), state, &buf, &len,
                                 &cap);
                i = j;
                continue;
            }
...
int expand_words(const struct token_vec *in, struct token_vec *out,
    ...
    for (i = 0; i < in->len; i++) {
        ...
        /* Preserve positional argument cardinality for "$@" helper code paths. */
        if (strcmp(in->items[i], "$@") == 0) {
            size_t j;
            for (j = 0; j < state->positional_count; j++) {
                out->items = xrealloc(out->items,
                                      sizeof(*out->items) * (out->len + 1));

...
static void clone_redirs(struct redir_vec *dst, const struct redir_vec *src) {
    size_t i;
...
static int prepare_runtime_redirections(struct shell_state *state,
                                        const struct redir_vec *src,
                                        struct redir_vec *dst) {
    dst->items = NULL;
    dst->len = 0;
    clone_redirs(dst, src);
    if (expand_redirection_operands(state, dst, NULL, NULL) != 0) {
        redir_vec_free(dst);
        return 1;
    }
    return 0;
...
static int execute_simple_command_parts(struct shell_state *state,
    ...
    clone_redirs(&redirs, ast_redirs);
    ...
    if (assign_count == words.len) {
        ...
        if (status == 0) {
            if (apply_redirections(state, &redirs, true, state->noclobber,
                                   false, &fd_backups) != 0) {
                status = 1;
            }
        }
...
static int run_group_with_redirection_source(struct shell_state *state,
    ...
    status = run_group_with_redirections(state, body, &redirs, run_body);
    redir_vec_free(&redirs);
    return status;
}
...
static int run_for_ast(struct shell_state *state, const struct ast_node *node) {
    ...
    if (node->data.for_cmd.redirs.len != 0) {
        if (prepare_runtime_redirections(state, &node->data.for_cmd.redirs,
                                         &runtime_redirs) != 0 ||
            apply_redirections(state, &runtime_redirs, true,
                               state->noclobber, false, &for_backups) != 0) {
...
static bool run_legacy_case_command(struct shell_state *state, char *trimmed,
    ...
    if (split_case_redirection_suffix(trimmed, &case_core, &case_redir_suffix)) {
        ...
        if (apply_redirections(state, &case_redirs, true, state->noclobber,
                               false, &case_backups) != 0) {
...
static int execute_ast_node(struct shell_state *state,
    ...
    case AST_NODE_SUBSHELL:
        status = run_subshell_group_ast(state, node);
        break;
    case AST_NODE_BRACE_GROUP:
        status = run_brace_group_ast(state, node);
        break;
    case AST_NODE_IF:
        status = run_if_ast(state, node);
        break;
    case AST_NODE_WHILE:
        status = run_loop_ast(state, node, false);
        break;
    case AST_NODE_UNTIL:
        status = run_loop_ast(state, node, true);
        break;
    case AST_NODE_FOR:
            ...
            if (part[0] != '\0') {
                ...
                if (apply_aliases) {
                    ...
                    if (alias_changed && ch != '\0' &&
                        command_text_needs_more_input(
                            part, strstr(part, "<<") != NULL)) {
                        size_t raw_len;
                ...
                state->errexit_ignored = false;
                {
                    if (ch == '&') {
                        ...
                        if (apply_aliases && alias_changed &&
                            ...
                            state->suppress_ast_aliases = true;
                            if (!try_run_ast_compound_command(state,
                                                              alias_cleaned,
                                                              true,
                                                              &status)) {

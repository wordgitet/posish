...
static int word_vec_push(struct word_vec *words, char *word) {
  words->items =
      xrealloc(words->items, sizeof(*words->items) * (words->len + 1));
...
static int collect_words_and_redirs(const struct token_vec *expanded,
...
static int apply_temporary_assignments(struct shell_state *state,
  ...
  for (i = 0; i < count; i++) {
    ...
    restore->items =
        xrealloc(restore->items, sizeof(*restore->items) * (restore->len + 1));
...
static int run_external_argv_vfork(struct shell_state *state, const char *path,
  ...
  if (pid == 0) {
    ...
    /*
     * execve failed — use _exit, never exit().
     * We cannot call perror() safely under vfork on all
     * platforms, but write(2) to stderr is safe.
     */
    {
  ...
  /* Parent continues here after child calls exec or _exit. */
  vars_free_envp(envp);
...
static int run_external_argv(struct shell_state *state, char *const argv[],
  ...
  /*
   * Fast path: use vfork when safe.  This avoids copying page tables
   * and gives a significant speedup on external command benchmarks.
   */
...
static int parse_redirections_from_source(const char *source,
...
static int execute_simple_command_parts(
    struct shell_state *state, const struct ast_word_vec *ast_raw_words,
    const struct redir_vec *ast_redirs, bool allow_builtin) {
  ...
  if (allow_builtin) {
    ...
    if (persist_builtin_redirs) {
      ...
      if (function_def != NULL) {
        ...
        if (status == 0) {
          status = execute_program_text(state, function_def->body);
        }
...
static int execute_simple_command(struct shell_state *state, const char *source,
  ...
  status =
      execute_simple_command_parts(state, &raw_words, &redirs, allow_builtin);
  return status;
}
...
static int run_group_with_redirection_source(struct shell_state *state,
  ...
  redirs.items = NULL;
  redirs.len = 0;
  if (parse_redirections_from_source(redir_source, state, &redirs) != 0) {
    redir_vec_free(&redirs);
    return 2;
  }
...
static int execute_program_text_internal(struct shell_state *state,
  ...
  if (strstr(source, "<<") != NULL) {
    ...
    if (try_run_ast_compound_command(state, source, true, &status)) {
      state->suppress_ast_aliases = saved_suppress_aliases;
      if (have_program_mark) {
      ...
      if (part[0] != '\0') {
        ...
        if (apply_aliases) {
          ...
          if (alias_changed && ch != '\0' &&
              command_text_needs_more_input(part, strstr(part, "<<") != NULL)) {
            size_t raw_len;
        ...
        state->errexit_ignored = false;
        {
          if (ch == '&') {
            ...
            if (!try_run_ast_compound_command(state, part, true, &status)) {
              status = 2;
            }
            state->suppress_ast_aliases = saved_suppress_aliases;
            state->last_status = status;
            ...
            if (apply_aliases && alias_changed &&
              ...
              saved_suppress_aliases = state->suppress_ast_aliases;
              state->suppress_ast_aliases = true;
              if (!try_run_ast_compound_command(state, alias_cleaned, true,
                                                &status)) {
                status =
                    execute_program_text_internal(state, alias_cleaned, false);
              }
              state->suppress_ast_aliases = saved_suppress_aliases;
